...
function checkConnection() {
    ...
    xhr.open('GET', base_url + 'server_status.php', true); // Replace 'server_status.php' with your hosting service URL
    xhr.onload = function() {
        if (xhr.status === 200) {
        var response = xhr.responseText;
        var alertDiv = document.getElementById('connectionAlert');
        if (response === 'Not Connected' && isConnected) {
        ...
        } else if (response === 'Connected' && !isConnected) {
            isConnected = true; // Update connection status
            if (!isAlertVisible) {
            isAlertVisible = false; // Set the alert visibility flag
            alertDiv.className = 'alert';
        ...
        } else if (response === 'Connected' && isConnected && isAlertVisible) {
                isConnected = true; // Update connection status
                isAlertVisible = true; // Set the alert visibility flag
                alertDiv.className = 'alert';
                alertDiv.style.backgroundColor = '#4CAF50';
