	...
	<table class="table text-center table-hover table-striped">
		...
		<tbody>
			<?php
				...
				$qry = $con->query("SELECT user.*, request.*, product.*, 
				...
				DATE_FORMAT(request.date_deleted, '%m-%d-%Y %h:%i:%s %p') as short_date_deleted
				FROM user 
				INNER JOIN request ON user.user_id = request.user_id 
				INNER JOIN product ON product.product_id = request.product_id 
				WHERE request.user_id = $user_id AND DATE(request.request_date) BETWEEN '{$from}' AND '{$to}' 
			...
			?>
				<tr>
					...
					<td class="">
						<p class="m-0">
							<?php if($row['request_updated'] == '0000-00-00 00:00:00'){
...
<script>
    ...
    exportBtn.addEventListener("click", function () {
        ...
        var filename = "export_announcement-" + date + ".csv";
...
<script>
	...
	function printPDF() {
		html2canvas(document.getElementById('capture-PDF')).then(function(canvas) {
			...
			var date = '<?php echo date("Ymd_His"); ?>';
			pdf.save("export_announcement-" + date + ".pdf");
		});
	}
</script>