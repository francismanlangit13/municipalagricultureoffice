	...
	<table class="table text-center table-hover table-striped">
		...
		<tbody>
			<?php 
				...
				FROM user INNER JOIN report where user.user_id = report.user_id AND date(date_created) between '{$from}' and '{$to}' order by unix_timestamp(date_created) asc");
...
<script>
    ...
    exportBtn.addEventListener("click", function () {
        ...
        var filename = "export_report-" + date + ".csv";
...
<script>
	...
	function printPDF() {
		html2canvas(document.getElementById('capture-PDF')).then(function(canvas) {
			...
			var date = '<?php echo date("Ymd_His"); ?>';
			pdf.save("export_report-" + date + ".pdf");
		});
	}
</script>